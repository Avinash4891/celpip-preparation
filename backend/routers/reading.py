...
from services.scoring import calculate_reading_score
...
def list_questions(
    ...
    query = db.query(Question).filter(Question.section == "reading")
    if part is not None:
...
def complete_session(
    ...
    session.score = calculate_reading_score(correct)
    db.commit()
    db.refresh(session)
    return session
